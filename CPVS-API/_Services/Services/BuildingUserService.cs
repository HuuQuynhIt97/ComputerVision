...
namespace CPVS_API._Services.Services
{
    public class BuildingUserService : IBuildingUserService
    {
        ...
        {
            var item = await _buildingUserRepository.FindAll().FirstOrDefaultAsync(x => x.UserID == userid);
            if (item == null)
            {
        ...
        {
            var item = await _buildingUserRepository.FindAll().FirstOrDefaultAsync(x => x.UserID == buildingUserDto.UserID);
            if (item == null)
            {
                _buildingUserRepository.Add(new BuildingUser { 
            ...
            } else
            {
                ...
                {
                    status = false,
                    message = "The User belonged with other OC!"
                };
            }
        }
